...
```{r}
jsoneg$result$codes$parliamentary_constituency
```
...
for (i in postcodes){
  ...
  #grab the JSON
  jsoneg <- fromJSON(url)
  #drill down into one branch and store it
...
```{r}
#This creates an empty vector called 'resultslist'
resultslist = c()
for (i in postcodes){
  url <- paste("http://api.postcodes.io/postcodes/",i, sep="")
  jsoneg <- fromJSON(url)
